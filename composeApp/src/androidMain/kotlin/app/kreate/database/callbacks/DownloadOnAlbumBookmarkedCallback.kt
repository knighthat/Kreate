...
class DownloadOnAlbumBookmarkedCallback : RoomDatabase.Callback(), KoinComponent {
    ...
    private val logger = Logger.withTag( "DownloadOnAlbumBookmarkedCallback" )
    private val scope: CoroutineScope by inject()
    private val table: AlbumTable by inject()
    private val mapTable: SongAlbumMapTable by inject()
    private val mediaDownloader: MediaDownloader by inject()
    private val snapshot = AtomicReference<Set<String>>(EMPTY_SET)

    private suspend fun start() {
        table.observedBookmarkedState()
            .collectLatest { newMap ->
                logger.v { "Album bookmark state changed" }

                // Check if this is first emit. If it is, only assign snapshot
                // and not download existing media items
                if( snapshot.load() === EMPTY_SET ) {
                    logger.d { "Initial run, capturing first snapshot of ${newMap.size} albums" }
                ...
                diff.filter { newMap[it] == -1L }
                    .forEach { id ->
                        ...
                        mapTable.allSongsOf( id )
                                .firstOrNull()
                                ?.forEach {
                                    val mediaItem = it.asMediaItem
                ...
                val downloads = mediaDownloader.downloads.value
                 ...
                 diff.filter { it !in downloads.keys || downloads[it]?.state != Download.STATE_COMPLETED }
                     // Convert Ids back to Song instance
                     .let { list ->
                         logger.d { "There are ${list.size} albums to be downloaded" }
                         list.flatMap { mapTable.allSongsOf(it).firstOrNull().orEmpty() }
                     }
    ...
    override fun onOpen( connection: SQLiteConnection ) {
        scope.launch {
            combine(Preferences.AUTO_DOWNLOAD, Preferences.AUTO_DOWNLOAD_ON_ALBUM_BOOKMARKED) { a, b -> a && b }
                .collectLatest {
                    if( !it ) {
                        logger.i { "Auto download on bookmark is set to off" }
                        return@collectLatest
                    } else {
